...
// checksum:   sha256:b24cae79f587ba3a0a66c5703b19c2f98a93837d7c176ee497f23a31664757a3
//
...
// GroupServiceImpl answers the port's questions straight from the store.
//
...
func NewGroupServiceImpl(repo *GroupRepository) *GroupServiceImpl {
...
func (s *GroupServiceImpl) ScopedService(ctx *configuration.AppContext) domain.Service {
	bound := *s
	bound.ctx = ctx
	return &bound
}
...
func (s *GroupServiceImpl) queryContext() context.Context {
	if s.ctx != nil {
		return s.ctx
	}
...
// GroupKeyTaken Reports whether another ACTIVE group in the same tenant
// already holds this key. Excludes the row being updated.
//
...
// enforce.
func (s *GroupServiceImpl) GroupKeyTaken(tenantID domain.ID, key string, selfID domain.ID) bool {
	conds := []criteria.Expr{
	...
	if err != nil {
		panic("Group: GroupKeyTaken probe failed")
	}
	return found
}

var (
	_ appdomain.GroupService            = (*GroupServiceImpl)(nil)
	_ persistence.ScopedServiceProvider = (*GroupServiceImpl)(nil)
)
