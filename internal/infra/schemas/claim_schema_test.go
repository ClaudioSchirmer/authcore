...
//
// The builder tests for claim_schema.go — they RUN the builder, so a boot
...
// checksum:   sha256:bed5ed0f5cf7cde50e2606dc670c6adce0f7501c416bb3feca933fd458b0fe64
//
...
// ClaimSchema must build without panicking, and must map a table.
//
...
// failed test.
func TestClaimBuilds(t *testing.T) {
	schema := ClaimSchema()
	if schema == nil {
		t.Fatal("ClaimSchema built a nil schema")
	}
	if schema.Table() == "" {
		t.Error("ClaimSchema built a schema with no table")
	}
}
