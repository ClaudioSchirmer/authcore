...
func (e *Role) BuildRules(actionName string, service domain.Service, r *domain.Rules) {
	...
	r.IfInsertOrUpdate(func() {
		// Storing the same permission twice would make a single revocation
		...
		// The cap is over the WHOLE collection — the number of permissions
		// one role bundles, with no grouping and no restriction. It is a
		// claim-size budget as much as a modelling one: the permission spec
		// sizes one rendered entry at 129 runes, so 200 is sized to this
		// platform rather than to GCP's 3000 or Azure's 2000.
		{
			items := domain.GetCurrentItemsOf[aggregatevos.RolePermission](e.GetAggregateRoot())
			if len(items) > 200 {
	...
	r.IfUpdate(func() {
		// The key is what API callers and audit lines reference; changing it
		// rewrites what every past reference meant, retroactively and
		// invisibly.
