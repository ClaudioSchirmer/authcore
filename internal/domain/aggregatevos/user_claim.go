...
//
// The UserClaim child value object.
//
// entity:     User
// spec:       specs/omnicore-gen/user.omnicore.yaml
// generator:  omnicore-gen
// generated:  2026-08-29
// checksum:   sha256:c31b0a93ff7f291a54e300755130b99d8338f659ac93b2502b2222709e0b9084
//
...
// UserClaim is one entry of User's claims collection.
//
// The claim values set on this user directly — level 1 of the two-level
...
type UserClaim struct {
	domain.Managed
	ClaimID domain.ID      `labelKey:"UserClaimClaimIDField"` // The claim definition this value belongs to — the id and not the name, so a retired-and-recreated definition needs an explicit re-set rather than silently inheriting a value written for the old one
	Value   vos.ClaimValue `labelKey:"UserClaimValueField"`   // The value this user carries for the definition above. Validated against the definition's declared ValueType by a rule, since no value object can read another aggregate's field
...
func (UserClaim) CollectionName() string { return "Claims" }
...
func (c UserClaim) IsSameBusinessIdentity(other domain.AggregateValueObject) bool {
	o, ok := other.(UserClaim)
	if !ok {
		return false
	}
...
// BuildRules validates one UserClaim.
//
...
func (c UserClaim) BuildRules(actionName string, service domain.Service, r *domain.Rules) {
