...
// The derivation is the single most consequential line in this package: it
// mints the value that becomes the tenant_id token claim and the FK target of
// every other aggregate.
func TestDeriveTenantIDIsDeterministicAndDistinct(t *testing.T) {
	const handle = "acme-comercio"
	first := TenantWorkspace(handle).DeriveTenantID()
	second := TenantWorkspace(handle).DeriveTenantID()

	if first != second {
		t.Fatalf("the derivation is not deterministic: %s then %s", first.Value(), second.Value())
	}
	if first.IsEmpty() {
		t.Fatal("the derivation produced an empty id")
	}
	if other := TenantWorkspace("acme-servicos").DeriveTenantID(); other == first {
		t.Error("two different handles derived the same public key")
	}
	// It must be a parseable UUID: the column is a native uuid on postgres.
	if _, err := first.UUID(); err != nil {
		t.Errorf("the derived id is not a UUID: %v", err)
	}
}

// THE NAMESPACE MUST NEVER CHANGE. Changing it re-derives every tenant_id in
// existence, invalidating every issued token and every foreign key pointing at
// one, with no migration path short of reissuing the whole platform's tokens.
//
// This test pins the namespace AND one derived value, so that a change to
// either is a red test rather than a silent platform-wide break.
func TestTenantIDNamespaceAndDerivationAreFrozen(t *testing.T) {
	const wantNamespace = "e2937874-80cb-4b5f-b113-21741931ac1a"
	if got := TenantIDNamespace.String(); got != wantNamespace {
		t.Fatalf("the tenant_id namespace moved: %s, want %s — this invalidates every issued token and every FK", got, wantNamespace)
	}
	// A frozen sample: UUIDv5 over the namespace above and this exact handle.
	const handle = "acme-comercio"
	const want = "427a1df1-e2a0-53a9-8f53-fe46edad93f1"
	if got := TenantWorkspace(handle).DeriveTenantID().Value(); got != want {
		t.Errorf("DeriveTenantID(%q) = %s, want %s — the derivation moved", handle, got, want)
	}
}
