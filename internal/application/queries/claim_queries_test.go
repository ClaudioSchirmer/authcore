...
// checksum:   sha256:aa791a243e7efb9f8910bbcdcfb3a710e4491c75792bc7ce1b66f117338fa114
//
...
func TestClaimCriteriaBuild(t *testing.T) {
	...
	if _, err := (FindClaimByIDQuery{}).ToCriteria(ctx); err != nil {
		t.Errorf("the by-id criteria failed: %v", err)
	}
	if _, err := (FindClaimsByParamsQuery{}).ToCriteria(ctx); err != nil {
...
func TestClaimReadsSurviveAnEmptyResult(t *testing.T) {
	ctx := &configuration.AppContext{}
	if _, err := (FindClaimByIDQuery{}).FromQueryResult(ctx, FindClaimByIDResult{}); err != nil {
		t.Errorf("the by-id derivation failed: %v", err)
	}
	if _, err := (FindClaimsByParamsQuery{}).FromQueryResult(ctx, FindClaimsByParamsResult{}); err != nil {
...
func TestClaimScopeIsForced(t *testing.T) {
	...
	out, err := (FindClaimsByParamsQuery{}).ToCriteria(anon)
	if err != nil {
...
func TestClaimBypassCrossesTheReadScope(t *testing.T) {
	ctx := &configuration.AppContext{}
	...
	out, err := (FindClaimsByParamsQuery{}).ToCriteria(ctx)
	if err != nil {
		t.Fatalf("the listing criteria failed: %v", err)
	}
...
func TestClaimQueriesShareAContext(t *testing.T) {
	for _, got := range []string{
		FindClaimByIDQuery{}.ContextName(),
		FindClaimsByParamsQuery{}.ContextName(),
	} {
		if got != "Claim" {
			t.Errorf("a query reports the context %q, the entity is %q", got, "Claim")
		}
	}
}
