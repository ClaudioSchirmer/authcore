...
// checksum:   sha256:504ce455a52ab46e113725f2e7388ee9cb11d34b5a4e53920009fa1e683d1de2
//
...
func TestClientCriteriaBuild(t *testing.T) {
	...
	if _, err := (FindClientByIDQuery{}).ToCriteria(ctx); err != nil {
		t.Errorf("the by-id criteria failed: %v", err)
	}
	if _, err := (FindClientsByParamsQuery{}).ToCriteria(ctx); err != nil {
...
func TestClientReadsSurviveAnEmptyResult(t *testing.T) {
	ctx := &configuration.AppContext{}
	if _, err := (FindClientByIDQuery{}).FromQueryResult(ctx, FindClientByIDResult{}); err != nil {
		t.Errorf("the by-id derivation failed: %v", err)
	}
	if _, err := (FindClientsByParamsQuery{}).FromQueryResult(ctx, FindClientsByParamsResult{}); err != nil {
...
func TestClientScopeIsForced(t *testing.T) {
	...
	out, err := (FindClientsByParamsQuery{}).ToCriteria(anon)
	if err != nil {
...
func TestClientBypassCrossesTheReadScope(t *testing.T) {
	ctx := &configuration.AppContext{}
	...
	out, err := (FindClientsByParamsQuery{}).ToCriteria(ctx)
	if err != nil {
		t.Fatalf("the listing criteria failed: %v", err)
	}
...
func TestClientQueriesShareAContext(t *testing.T) {
	for _, got := range []string{
		FindClientByIDQuery{}.ContextName(),
		FindClientsByParamsQuery{}.ContextName(),
	} {
		if got != "Client" {
			t.Errorf("a query reports the context %q, the entity is %q", got, "Client")
		}
	}
}
