...
//
// The projectors for the permissions collection.
//
// entity:     Role
// spec:       specs/omnicore-gen/role.omnicore.yaml
// generator:  omnicore-gen
// generated:  2026-09-01
// checksum:   sha256:5e8e6fd2c885b2bb1a7f83cd252c45778daae9f3bbc3c062a1f7f5a95fd3f5c2
//
...
// ProjectRolePermissions reads Role's permissions back off the saved
// aggregate.
//
// Every verb that answers with the whole aggregate calls this one, which is
// why it is here and not in any of their files.
func ProjectRolePermissions(e *appdomain.Role) []cmddtos.RolePermissionResult {
	items := domain.GetCurrentItemsOf[aggregatevos.RolePermission](&e.AggregateRoot)
	out := make([]cmddtos.RolePermissionResult, 0, len(items))
	for _, item := range items {
		out = append(out, cmddtos.RolePermissionResult{
			ID:           item.GetID(),
...
func ProjectOneRolePermission(item aggregatevos.RolePermission) cmddtos.RolePermissionResult {
	return cmddtos.RolePermissionResult{
		ID:           item.GetID(),
		PermissionID: item.PermissionID,
	}
}
