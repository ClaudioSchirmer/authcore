...
// checksum:   sha256:23b84b3ab91c993c72f620e40171b1a2a6acddd1c89051096e443fb908ad96cb
//
...
// ProjectClientRoles reads Client's roles back off the saved aggregate.
//
// Every verb that answers with the whole aggregate calls this one, which is
// why it is here and not in any of their files.
func ProjectClientRoles(e *appdomain.Client) []cmddtos.ClientRoleResult {
	items := domain.GetCurrentItemsOf[aggregatevos.ClientRole](&e.AggregateRoot)
	out := make([]cmddtos.ClientRoleResult, 0, len(items))
	for _, item := range items {
		out = append(out, cmddtos.ClientRoleResult{
			ID:     item.GetID(),
			RoleID: item.RoleID,
		})
	}
...
func ProjectOneClientRole(item aggregatevos.ClientRole) cmddtos.ClientRoleResult {
	return cmddtos.ClientRoleResult{
		ID:     item.GetID(),
		RoleID: item.RoleID,
	}
}
