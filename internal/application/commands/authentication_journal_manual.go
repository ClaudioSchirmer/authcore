...
// authenticationJournal records what happened to an authentication attempt, in
...
// newAuthenticationJournal binds the ports to one subject kind.
func newAuthenticationJournal(
	attempts AttemptRecorder, events AuthenticationEventPublisher, kind string,
) authenticationJournal {
	return authenticationJournal{attempts: attempts, events: events, kind: kind}
}

// lockedUntil asks whether this identity is refused outright, and hands back what
...
func (j authenticationJournal) lockedUntil(
...
func (j authenticationJournal) refusedWhileLocked(
...
func (j authenticationJournal) failed(
...
func (j authenticationJournal) succeeded(ctx *configuration.AppContext, identity, ip string) error {
...
// out".
func (j authenticationJournal) values(identity, ip string, identityExisted *bool) map[string]any {
	vals := map[string]any{
		"identity":     identity,
		"identityKind": j.kind,
		"ip":           ip,
...
func (j authenticationJournal) announce(
