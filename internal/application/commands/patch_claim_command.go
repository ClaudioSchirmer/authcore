...
// checksum:   sha256:9927660121ff3a213b1c0692a80cc111aa14ce071bcca66e545a8970cf71ed13
//
...
// PatchClaimCommand carries the writable fields of the request.
//
...
func (c *PatchClaimCommand) ApplyPartiallyTo(ctx *configuration.AppContext, e *appdomain.Claim) error {
	if c.AppliesTo != nil {
...
// PatchClaimResult is the write response, projected from the entity.
type PatchClaimResult struct {
	ID           domain.ID
	TenantID     domain.ID
	Name         string
	ValueType    string
...
// caller.
func (c *PatchClaimCommand) FromEntity(_ *configuration.AppContext, e *appdomain.Claim) (PatchClaimResult, error) {
	return PatchClaimResult{
		ID:           *e.GetID(),
		TenantID:     e.TenantID,
		Name:         e.Name.Value(),
