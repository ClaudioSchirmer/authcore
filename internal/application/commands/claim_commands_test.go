...
// checksum:   sha256:8e3977721f184736de65b5211a3343fd4f1118b964c4627164cd2b820caefc72
//
...
func TestInsertClaimMapsEveryField(t *testing.T) {
	...
	if e.ValueType.Value() != "string" {
		t.Errorf("ValueType did not survive the mapper")
	}
	if e.AppliesTo.Value() != "both" {
		t.Errorf("AppliesTo did not survive the mapper")
	}
...
func TestInsertClaimCommandResultCarriesWhatWasWritten(t *testing.T) {
	ctx := &configuration.AppContext{}
	c := &InsertClaimCommand{
		Name:         "x_cost_center",
		ValueType:    "string",
	...
	if res.Name != "x_cost_center" {
		t.Errorf("Name did not reach the result")
	}
	if res.ValueType != "string" {
		t.Errorf("ValueType did not reach the result")
	}
	if res.AppliesTo != "both" {
		t.Errorf("AppliesTo did not reach the result")
	}
...
// ArchiveClaimCommand must apply cleanly to a well-formed entity.
//
...
func TestArchiveClaimCommandApplies(t *testing.T) {
	ctx := &configuration.AppContext{}
	// A request has a caller. With no identity the mappers' identity feed is
	// skipped entirely, and what a scoped write is checked against is exactly
	// what the feed carries.
