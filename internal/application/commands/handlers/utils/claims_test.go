...
// usableAccount is this package's own fixture.
//
// It used to be borrowed from the handler suite one level up; the claim chain moved
// down here and a test must not reach across a package boundary for a struct
// literal. Only the fields the chain reads are set — everything the HANDLER cares
// about (the credential, the statuses, the tenant join) is that suite's business.
