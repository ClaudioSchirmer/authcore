...
func ResolveCustomClaims(account *schemas.SignInAccount, bundle infra.SignInBundle) map[string]any {
	...
	return ResolveClaimChain(account.MustChangePassword, bundle.ClaimValues, bundle.Definitions)
}

// ResolveClaimChain is the chain itself, taking only what it reads.
//
// IT IS THE SEAT BOTH TOKEN PATHS SHARE, and that is why it takes three plain
// arguments instead of an account and a bundle: POST /auth/client/token resolves
// the same two levels over `client_claims` and the `client`/`both` half of the same
// catalog, and a second copy of this walk would be a second reading of what a
// `bool` is, what an absent value means, and how the budget is spent. There is no
// type both callers could hand in that would not have to be invented for the
// purpose — see the domain-membership rule; three primitives cross the seam
// instead.
//
// restricted is the caller's own reading of "this session carries nothing". On the
// user path it is mustChangePassword; on the client path there is no such state and
// it is always false — a machine has no credential to be told to rotate.
func ResolveClaimChain(
	restricted bool, held map[domain.ID]string, definitions []schemas.ClaimDefinition,
) map[string]any {
