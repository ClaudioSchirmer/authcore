...
func rotateCmd(grace *int) *commands.RotateClientSecretCommand {
	cmd := &commands.RotateClientSecretCommand{GracePeriodSeconds: grace}
...
// clamped, and the credential the caller is using must survive the utils.Refusal.
...
func TestTheInsertResultCarriesTheMintedSecret(t *testing.T) {
	...
	c := &commands.InsertClientCommand{
