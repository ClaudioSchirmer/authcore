...
func resetCmd() *commands.ResetPasswordCommand {
	cmd := &commands.ResetPasswordCommand{Password: newPassword, PasswordConfirmation: newPassword}
...
func TestTheResetStillEnforcesThePolicy(t *testing.T) {
	...
	cmd := &commands.ResetPasswordCommand{Password: "short", PasswordConfirmation: "short"}
	cmd.SetPathID(targetUserID)
...
func TestTheResetComparesTheConfirmation(t *testing.T) {
	...
	cmd := &commands.ResetPasswordCommand{Password: newPassword, PasswordConfirmation: "An0ther!Passphras"}
	cmd.SetPathID(targetUserID)
...
func TestTheResetRefusesTheCurrentPassword(t *testing.T) {
	...
	cmd := &commands.ResetPasswordCommand{Password: oldPassword, PasswordConfirmation: oldPassword}
	cmd.SetPathID(targetUserID)
...
// credentialRefusals names the notifications a utils.Refusal raised, by type.
//
...
// raised reports whether the utils.Refusal carries the given notification type, named
...
func changeCmd(current, next string) *commands.ChangePasswordCommand {
...
// door, so the tenant guard says nothing — the utils.Refusal has to come from the
...
func TestTheChangeRefusesARowThatIsNotTheCallers(t *testing.T) {
	...
	if !raised(err, "PasswordChangeRequiresSelfNotification") {
		t.Fatalf("the utils.Refusal did not come from the row decision: %v", credentialRefusals(err))
	}
	if !hasher.Matches(oldPassword, store.user.PasswordHash) {
...
func TestTheChangeRefusesAWrongCurrentPassword(t *testing.T) {
	...
	if !raised(err, "InvalidCurrentPasswordNotification") {
		t.Fatalf("the utils.Refusal did not come from the current-password proof: %v", credentialRefusals(err))
	}
	if !hasher.Matches(oldPassword, store.user.PasswordHash) {
		t.Fatal("a refused change rotated the credential anyway")
	}
...
func TestTheChangeRefusesTheCurrentPasswordAsTheNewOne(t *testing.T) {
	...
	if !raised(err, "PasswordUnchangedNotification") {
		t.Fatalf("the utils.Refusal did not come from the must-differ rule: %v", credentialRefusals(err))
	}
}
...
func TestTheChangeComparesTheConfirmation(t *testing.T) {
	...
	if !raised(err, "PasswordConfirmationMismatchNotification") {
		t.Fatalf("the utils.Refusal did not come from the confirmation rule: %v", credentialRefusals(err))
	}
}
...
// utils.Refusal: the caller is authenticated and already holds the permission, so
