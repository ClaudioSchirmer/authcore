...
func TestTokenResponse_FromResult(t *testing.T) {
	result := commands.TokenResult{
		...
		User: commands.AuthenticatedUserResult{
			...
			Groups:             []commands.NamedGrantResult{{Key: "eng", Name: "Engineering"}},
			Roles:              []commands.NamedGrantResult{{Key: "viewer"}},
...
func TestNamedGrantResponse_OmitsAnAbsentName(t *testing.T) {
	encoded, err := json.Marshal(NamedGrantResponse{Key: "viewer"})
	if err != nil {
		t.Fatalf("marshalling: %v", err)
	}
