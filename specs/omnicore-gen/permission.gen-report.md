...
- **`PermissionKey`** — `internal/domain/vos/permission_key.go`. A permission: what resource, and what may be done to it. The resource is a colon-joined path of 2-64 rune lowercase slugs (tenant, user:profile); the action is exactly one such slug, so the LAST colon segment is always the action and the rendering parses back exactly one way. The wildcard * is accepted only as an ENTIRE part — never inside a path, never mixed into a slug — because the claim matcher honors exactly three shapes: an exact string, resource:*, and *:*. A * resource therefore forces a * action. The rendered token is bounded at 129 runes by construction, since each part is bounded at 64. No normalization: a value that does not already comply is refused, never repaired, because it is compared byte-for-byte against a token claim. Renders itself through String().
...
> The Description must differ from the rendered resource:action string under a normalized comparison — case-folded, with whitespace, colons and hyphens collapsed — which catches the lazy paste and nothing more. Render the key through Key.String(); nothing else in this service joins a resource and an action.
...
> The permission as a JWT claim carries it and RequirePermission compares it — resource:action, rendered from the two stored columns.
...
| Unique | `Resource` — scope `active-only` (service-precheck+constraint) | an archived row frees it, so the value can be taken again; a duplicate is refused at the database and reported as `PermissionAlreadyExistsNotification`. |
