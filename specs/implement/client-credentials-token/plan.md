...
| `internal/infra/schemas/client_sign_in_schemas_test.go` **(new)** | each schema agrees with the generated schema of the same table; no repository is anchored on a join target | — |
...
| `internal/application/commands/handlers/dtos/ports.go` | += `ClientAuthenticationStore`, `AccessTokenIssuer` | `service-layout.html`; `shared/domain-membership.md` |
