...
enforce — dozens — and capped at 100 rows per page. `tenants`, which grows with the
...
| `GET /permissions` · `GET /permissions/:id` · `permissions` · `permission` | `permission:read` |
  ...
  catalog**, and those rows come from the bootstrap seed migration (§B Q4, superseded
  2026-09-01) rather than from an operator's first call — which is what makes a fresh
