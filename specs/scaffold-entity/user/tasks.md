...
| 6 | §B Q2 / §10: the reset accepts **self**, `*:*`, or `user:reset-password` | it accepts `user:reset-password` — which `*:*` satisfies for free — and **not self** | `auth.authorization.enabled=true` makes the framework refuse to boot with a non-public route declaring no permission (`bootstrap/route_scan.go`, proven by booting a profile with the flag on and reading the panic), and `RequirePermission` expresses exactly ONE permission. Three alternatives were not declarable. The security argument points the same way: a user who KNOWS their password had the change endpoint, and one who does not is who needs a helpdesk |
| 7 | — | the reset handler feeds the aggregate's row-scope guard | not in the spec at all, and it was missing: `refuseForeignTenant` asks whether an identity was PRESENT before comparing, so a hand-written command that skips those three lines leaves it standing down. A holder of `user:reset-password` in one tenant could have reset a password in another. Found while applying deviation 6; covered by a test |
| 8 | — | there is **no self-service password change** | the consequence of 5 and 6 together, and the one gap this entity ships with. It needs either a forgot-password flow or an authenticated change carrying the current password; the maintainer deferred both. Recorded as its own row in the README's state table so it is not mistaken for an oversight |
