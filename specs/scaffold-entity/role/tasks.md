...
| 4 | §1's ER sketch shows `UNIQUE (role_id, permission_id) WHERE archived_at IS NULL` | Present, **hand-written** in the migration | The generator emits the repository's 409 binding for it but not the DDL: migrations are write-once hook files. The index NAME must match the binding exactly or the violation degrades to a raw 500 |
