...
COMMENT ON COLUMN "permissions"."resource_name" IS 'The thing being protected, as the enforcing route names it. A single slug or a colon-joined path; * means every resource.';
COMMENT ON COLUMN "permissions"."action_name" IS 'What may be done to the resource — exactly one slug, never a path; * means every action on that resource.';
