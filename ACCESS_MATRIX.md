...
| **Which address** | The one this service sees on the SOCKET. The framework reads no proxy header — it sets neither `TrustProxy` nor `ProxyHeader` on the Fiber app — so **nothing a caller sends can forge it**. The flip side: behind an ingress or a load balancer it is the BALANCER's address, so an allow-list of real egress ranges refuses everybody and one holding the balancer's range admits everybody. On such a deployment leave the collection empty until omnicore ships a trusted-proxy block |
...
**The catalog is capped, and the cap is per identity KIND.** At most **20 ACTIVE definitions per tenant** may admit a user (`appliesTo` `user` or `both`), and at most 20 may admit a client (`client` or `both`). The two buckets OVERLAP — a `both` definition spends a slot on each — so two full buckets are between 20 and 40 rows, and a `user`-only definition costs a client token nothing. The 21st answers **422** naming which bucket is full. The cap asks only about the kinds a write ADDS: an insert, or a widening of `appliesTo`. An edit that leaves `appliesTo` alone is never refused by it, which is what keeps a tenant already over the line — seeded rows, direct SQL — able to repair its own catalog. Archiving frees the slot immediately. It is the same 20 the two per-principal caps carry, and it is what makes the token's 20-per-token truncation unreachable through the API: the bucket it caps is the same predicate the emission walks.
...
| Create a 21st claim definition admitting a given identity kind in one tenant | **422** | 20 per tenant per kind — `user`+`both` on one side, `client`+`both` on the other, so the two buckets overlap |
