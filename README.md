...
**Two levels, and this entity is the second one.**
...
`user:set-claim` and `client:set-claim`.

**The chain now reaches a USER's token**, resolved on 2026-08-28 in its own run
(`specs/implement/emit-custom-claims-on-user-token/plan.md`). `POST /auth/user/token` and its
refresh walk the tenant's ACTIVE catalog, overlay the values set on the user, and mint what
survives beside the fixed nine — each value in the JSON type its definition declares, through
the same `ClaimValueMatchesValueType` gate both write levels ask. Three consequences are worth
naming because none of them is obvious from the diagram: the walk is over the CATALOG and not
over the user's entries, so an **archived definition mints nothing** even for a principal who
still holds a value for it; at most **20 claims ride on one token**, values set on the user
spent before any tenant-wide default and the rest dropped with a named `Warn`; and a
...
**The catalog is capped: 20 ACTIVE definitions per tenant, per identity kind** — at most 20 may
carry `appliesTo` `user` or `both`, and at most 20 may carry `client` or `both`. The two buckets
**overlap**, so a `both` definition spends a slot on each side and two full buckets are anywhere
between 20 and 40 rows; a `user`-only definition costs a client token nothing. The 21st is
refused at the write with a 422 that names *which* bucket is full.

The number is the same 20 the two per-principal caps carry and the same 20 a token spends, and
that is the point rather than a coincidence: the emission's candidate set is exactly
...
is already over the line (seeded rows, direct SQL) able to repair its own catalog instead of
being locked out of it. Archiving a definition frees its slot immediately, on both sides if it
was `both`.
